...
async def get_current_user(Authorize: AuthJWT = Depends()):
    ...
    if user_in_db:
        return User(
            ...
            monitered_papers=getKeyOrDefault(
                user_in_db, "monitered_papers", default=[]
            ),
...
def register(
    userData: UserRegistrationForm = Depends(UserRegistrationForm.as_form),
    ...
    if user_created_status == -1:
        ...
        is_sent = authService.send_verification_email(
            userData.full_name, userData.email, token
        )
...
@router.post("/login")
async def login(
    email: str = Form(...), password: str = Form(...), Authorize: AuthJWT = Depends()
...
async def password_reset_email(email: str = Form(...), Authorize: AuthJWT = Depends()):
    ...
    token = Authorize.create_access_token(
        subject=email
    )  # authService.create_user_access_token(email, SECRET_KEY)
    authService.send_password_reset_email(email, token)
    return {"success": True}


@router.post("/reset_password", dependencies=[Depends(JWTBearer())])
async def loged_in_reset_password(
    new_password: str = Form(...), Authorize: AuthJWT = Depends()
):
    Authorize.jwt_required()
    email = (
        Authorize.get_jwt_subject()
    )  # authService.get_email_from_token(token, SECRET_KEY)
...
async def create_collection(
    collection_name: str = None, user_in_db: UserInDB = Depends(get_current_user_in_db)
):
    res_status = user_in_db.create_collection(collection_name)
    if res_status == -1:
        raise HTTPException(
            status_code=404, detail="Collection already exist",
        )
...
async def rename_collection(
    collection_name,
    new_collection_name,
    user_in_db: UserInDB = Depends(get_current_user_in_db),
):
    res_status = user_in_db.rename_collection(collection_name, new_collection_name)
    if res_status == -1:
        raise HTTPException(
            status_code=404, detail="collection_name not found",
        )
...
async def delete_collection(
    collection_name: str, user_in_db: UserInDB = Depends(get_current_user_in_db)
):
...
async def delete_collection_paper(
    pid: str,
    collection_name: str,
    user_in_db: UserInDB = Depends(get_current_user_in_db),
):
    ...
    if res_status == -1:
        raise HTTPException(
            status_code=404, detail="collection_name not found or pid not found",
        )
...
async def add_moniter_paper(
    pid: str, user_in_db: UserInDB = Depends(get_current_user_in_db)
):
...
async def delete_moniter_paper(
    pid: str, user_in_db: UserInDB = Depends(get_current_user_in_db)
):
...
async def correct_metadata_request(
    correct_meta: PaperMetadataCorrection, Authorize: AuthJWT = Depends()
...
def get_correct_metadata_request():
    ...
    results = PaperMetadataCorrectionES.search(
...
def unlist_paper(Authorize: AuthJWT = Depends()):
    ...
    search_request = elastic_models.UserRequest.search(
        using=elastic_service.get_connection()
    )
...
def unlist_paper(Authorize: AuthJWT = Depends()):
    ...
    search_request = search_request.filter(
        "terms", status=["COMMITTED", "DENIED", "MODIFIED"]
    )
    result = search_request.execute()
    response = []
    for hit in result:
...
def unlist_paper(req: ProcessRequest, Authorize: AuthJWT = Depends()):
    ...
    user_request = elastic_models.UserRequest.get(
        id=req.request_id, using=elastic_service.get_connection()
    )
