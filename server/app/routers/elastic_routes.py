...
def paper_list(mget_request: MGetRequest):
    ...
    return SearchQueryResponse(
        query_id=str(uuid4()), total_results=total_results, response=result_list
    )
...
def citations(request: Request, id: str, sortBy: str, page: int = 1, pageSize: int = 10):
    ...
    for doc_hit in response["hits"]["hits"]:
        result_list.append(
            build_citation_entity(_id=doc_hit["_id"], doc=doc_hit["_source"])
        )
    total_results = response["hits"]["total"]["value"]
...
def show_citing_papers(request: Request, cid: str, sort: str, page: int, pageSize: int):
    ...
    if len(papers_that_cite) > 0:
        search = (
            elastic_models.Cluster.search(using=elastic_service.connection)
        ...
        total_results = response["hits"]["total"]["value"]
    return showCitingClustersResponse(
        query_id=str(uuid4()),
        total_results=total_results,
        cluster=primary_cluster_detail,
        papers=result_list,
    )
...
def _get_sort_param(param: str) -> dict:
    if param == "yearAsc":
        ...
        return {
            "_script": {
                "type": "number",
                "script": {"lang": "painless", "source": "doc.cited_by.length"},
                "order": "desc",
            }
        }
    else:
...
def get_suggestions(request: Request, query: str):
    ...
    for option in response.suggest.auto_complete[0].options:
        suggestions.append(
            Suggestion(type="paper", text=option._source.title, id=option._id)
        )
    return AutoCompleteResponse(
        query_id=str(uuid4()), query=query, suggestions=suggestions
    )
...
def similar_papers(request: Request, id: str, algo: str):
    res = None
    if algo == "Co-Citation":
        s = elastic_models.Cluster.search(
            using=elastic_service.get_connection(), index="clusters_next"
        ).filter("match", cited_by=id)
        res = s.execute()
    ...
    for doc in res["hits"]["hits"]:
        result_list.append(
            build_similar_papers_entity(_id=doc["_id"], doc=doc["_source"])
        )
...
def search_facet(searchQuery: SearchFilter):
    ...
    s = s.query(
        "nested",
        path="authors",
        query=Q("multi_match", query=q, fields=["authors.fullname"]),
    )
    response = s.execute()
    result_list, res = [], []
    tofilter = q.split(" ")
    ...
    return SearchAuthorResponse(
        query_id=str(uuid4()), total_results=total_results, response=res
    )
...
def delete_paper(request: Request, id: str):
    ...
    ubq = (
        UpdateByQuery(index=settings.CLUSTERS_INDEX)
        .using(elastic_service.get_connection())
        .filter("term", cited_by=id)
        .script(
            source="""
    ...
    search_cluster = elastic_models.Cluster.search(
        index=settings.CLUSTERS_INDEX, using=elastic_service.get_connection()
    )
...
def get_authors_in_list(doc, field) -> List[str]:
    return [
        getKeyOrDefault(field, "fullname", default="")
        for field in getKeyOrDefault(doc, field, default={})
    ]
...
def build_facets(agg_dict_yr, agg_dict_pub, agg_dict_athr, agg_min_year):
    return Facets(
        minimum_year=getKeyOrDefault(
            getKeyOrDefault(agg_min_year, "min_year"), "value"
        ),
        pub_info_year_count=getKeyOrDefault(
            getKeyOrDefault(agg_dict_yr, "pub_info_year_count"), "value"
        ),
        pub_info_year_list=get_aggregation_list(
            getKeyOrDefault(
                getKeyOrDefault(agg_dict_yr, "pub_info_year_list"), "buckets"
            )
        ),
        pub_info_publisher_count=getKeyOrDefault(
            getKeyOrDefault(agg_dict_pub, "pub_info_publisher_count"), "value"
        ),
        pub_info_publisher_list=get_aggregation_list(
            getKeyOrDefault(
                getKeyOrDefault(agg_dict_pub, "pub_info_publisher_list"), "buckets"
            )
        ),
        authors_count=getKeyOrDefault(
            getKeyOrDefault(agg_dict_athr, "authors_count"), "value"
        ),
        authors_fullname_terms=get_aggregation_list(
            getKeyOrDefault(
                getKeyOrDefault(agg_dict_athr, "authors_fullname_terms"), "buckets"
            )
        ),
    )
