...
#         return self.elastic_service.paginated_search('papers_next_new2', searchQuery.queryString, searchQuery.page,
#                                                      searchQuery.pageSize, ['title', 'text'])
#
#     def get_sorted_papers(self, papers_list, page, pageSize, sort):
#         """Given Cluster IDs get them sorted in desired order with Pagination support"""
#         if sort == "yearAsc":
...
#         return self.elastic_service.paginated_search(index='paper_new_next', query=id, page=page, pageSize=pageSize,
#                                                      fields_to_search='cites')
#
#
...
#
#     def cluster_paper(self, keys: List[str], paper: Cluster):
#         """
...
#
#     def _update_cluster_for_paper(self, cluster_id: str, paper: Cluster, keys: List[str]):
...
#         cluster.save(using=self.elastic_service.get_connection())
#
#     def _create_new_cluster_for_paper(self, paper: Cluster, keys: List[str]):
#         paper.save(using=self.elastic_service.get_connection())
#         cluster = Cluster(
#             title=paper.title,
