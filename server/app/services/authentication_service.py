...
class AuthenticationService:
    ...
    def verify_password(
        self, salt: str, plain_password: str, hashed_password: str
    ) -> bool:
    ...
    def create_admin(self, admin_data: AdminUser):
        ...
        admin_in_db = AdminInDB(
            salt=salt, username=admin_data.username, hashed_password=hashed_password
        )
    ...
    def authenticate_user(self, email: str, password: str) -> (int, UserInDB):
        ...
        if not user or not self.verify_password(
            user.salt, password, user.hashed_password
        ):
            return -1, None
    ...
    def add_moniter_paper(self, user_in_db: UserInDB, pid):
        if (
            hasattr(user_in_db, "monitered_papers")
            and pid not in user_in_db.monitered_papers
        ):
    ...
    def delete_moniter_paper(self, user_in_db: UserInDB, pid):
        if (
            hasattr(user_in_db, "monitered_papers")
            and pid in user_in_db.monitered_papers
        ):
    ...
    def correct_metadata_request(
        self, correct_meta: PaperMetadataCorrection, user_email: str
    ):
        ...
        for author in correct_meta.authors:
            ...
            correct_meta_ES.authors.append(
                Author(
